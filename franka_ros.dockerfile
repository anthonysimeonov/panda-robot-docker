...
RUN sh /tmp/install_basic.sh

# install ROS stuff
ENV ROS_DISTRO noetic

COPY ./install_scripts/install_ros.sh /tmp/install_ros.sh
RUN sh /tmp/install_ros.sh

# bootstrap rosdep
RUN rosdep init && rosdep update

# install panda
RUN apt update && apt install -y cmake git libpoco-dev libeigen3-dev ros-noetic-catkin python3-catkin-tools 
...
# install LCM
RUN apt update && apt install libglib2.0-dev freeglut3-dev -y
...
RUN apt-get install -y ros-noetic-realsense2-camera ros-noetic-rgbd-launch
...
RUN usermod -a -G realtime ${USER_NAME}
